...
namespace Entities
{
    public class Vendedores
    {
        ...
        public bool ValidaVendedor(TextBox tbVendedor, TextBox tbEmail, MaskedTextBox mtbTelefone, MaskedTextBox mtbWhats, ErrorProvider ep)
        {
            List<bool> erroList = new List<bool>();
            erroList.Add(VNome(this.Nome, tbVendedor, ep));
        ...
        private bool VTelefone(string numero, MaskedTextBox mtbTelefone, ErrorProvider ep)
        {
            ...
            if (numero.Length == 0)
            {
                ep.SetError(mtbTelefone, "Digite um Numero para o Vendedor");
                errosList.Add(false);
            }
        ...
        private bool VNome(string texto, TextBox tbNome, ErrorProvider ep)
        {
            ...
            {
                ep.SetError(tbNome, "Digite o nome do Vendedor");
                errosList.Add(false);
            }
            if (regex.IsMatch(texto))
            {
                ep.SetError(tbNome, "O nome do Vendedor não deve possuir numeros ou Caractéres especiais");
                errosList.Add(false);
            }
            if (texto.Trim().Length > 0 && texto.Trim().Length < 10)
            {
                ep.SetError(tbNome, "Digite o Nome e o Sobrenome do Vendedor");
                errosList.Add(false);
            }
            if (Invalida(errosList))
            {
